...
import { MAPBOX, OPENWEATHER } from '@/v2/weather/constants.js';
...
export default new Hono()
  ...
  .get('/weather', withWeatherLanguage, async (c) => {
    ...
    if (lat && lon) {
      ...
      url = `${OPENWEATHER}/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${c.env.OPENWEATHER_TOKEN}&lang=${c.get('language')}`;
    } else if (city) {
      url = `${OPENWEATHER}/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${c.env.OPENWEATHER_TOKEN}&lang=${c.get('language')}`;
    } else {
